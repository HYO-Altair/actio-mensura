...
const ref = db.ref("SERVER1 CHAN1");
...
const servername = "SERVER1"; // change to test output
const channelname = "CHAN1"; // change to test output
const username = "USER1 0000"; // change to test output
// if server exists
db.ref(`servers/${servername}`)
  ...
  .then((snapshot1) => {
    if (snapshot1.val()) {
      ...
      const messagesRef = db.ref(`${servername} ${channelname}/messages`);
